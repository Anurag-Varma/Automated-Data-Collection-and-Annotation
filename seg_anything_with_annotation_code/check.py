...
def save_recent_mask_to_file(undo_stack,extrinsics1,extrinsics2,image2,label,rsObj,in_params2,in_model2,in_coeff2):
    ...
    # Printing the first pose of the camera with respect to the base reference frame:
    print('Pose 1: ', extrinsinc_pos1)
    ...
    print('Rotation matrix of the first camera pose: ', R_pose_1)
    print('Position vector of the first camera pose: ', p_pose_1)
    ...
    print('Shape of the numpy array points: ', result_arr.shape)
    ...
    cloud_object_transformed_points.g_base_cam = extrinsinc_pos1

    # Printing the first pose of the camera with respect to the base reference frame:
    print('Pose 2: ', extrinsinc_pos2)
    # Extracting the rotation matrix and position vector: 
    R_pose_2 = extrinsinc_pos2[0:3, 0:3]
    p_pose_2 = np.reshape(extrinsinc_pos2[0:3, 3], [3,1])
    print('Rotation matrix of the first camera pose: ', R_pose_2)
    print('Position vector of the first camera pose: ', p_pose_2)

    cloud_object_transformed_points.R_base_cam = R_pose_2
    cloud_object_transformed_points.p_base_cam = p_pose_2

    # Converting the 'transformed_points' list to a numpy array 
    num_points = len(transformed_points)
    transformed_points = np.reshape(np.asarray(transformed_points), [num_points, 3])
    print('Shape of the numpy array points: ', transformed_points.shape)
    
    '''Creating a Open3d PointCloud Object for the cloud corresponding to just the bounding box'''
    objectCloud = o3d.geometry.PointCloud()
    objectCloud.points = o3d.utility.Vector3dVector(transformed_points.astype(np.float64))
    objectCloud.paint_uniform_color([0, 0, 1])
    ...
    cloud_object_transformed_points.points = np.asarray(cloud_object_transformed_points.processed_cloud.points)
    cloud_object_deprojected_points.points = np.asarray(cloud_object_deprojected_points.processed_cloud.points)

    x_points_transformed_points = np.reshape(cloud_object_transformed_points.points[:, 0], [cloud_object_transformed_points.points.shape[0],1])
    y_points_transformed_points = np.reshape(cloud_object_transformed_points.points[:, 1], [cloud_object_transformed_points.points.shape[0],1])
    z_points_transformed_points = np.reshape(cloud_object_transformed_points.points[:, 2], [cloud_object_transformed_points.points.shape[0],1])


    x_points_deprojected_points = np.reshape(cloud_object_deprojected_points.points[:, 0], [cloud_object_deprojected_points.points.shape[0],1])
    y_points_deprojected_points = np.reshape(cloud_object_deprojected_points.points[:, 1], [cloud_object_deprojected_points.points.shape[0],1])
    z_points_deprojected_points = np.reshape(cloud_object_deprojected_points.points[:, 2], [cloud_object_deprojected_points.points.shape[0],1])

    '''Plot 1: '''
    '''Visualizing the points after transformation between the two camera reference frames: '''
    fig1 = plt.figure()
    ...
    ax1.scatter(x_points_transformed_points, y_points_transformed_points, z_points_transformed_points, s = 0.2)
    
    '''Base reference Frame: '''
    cloud_object_transformed_points.R = cloud_object_transformed_points.R_base
    cloud_object_transformed_points.p = cloud_object_transformed_points.p_base
    cloud_object_transformed_points.scale_value = 0.45
    cloud_object_transformed_points.length_value = 0.25
    ax1 = cloud_object_transformed_points.plotReferenceFrames(ax1)

    '''Camera reference Frame:'''
    cloud_object_transformed_points.R = R_pose_2
    cloud_object_transformed_points.p = p_pose_2
    cloud_object_transformed_points.scale_value = 0.25
    cloud_object_transformed_points.length_value = 0.15
    ax1 = cloud_object_transformed_points.plotReferenceFrames(ax1)

    ax1.set_xlim(-0.6, 0.9)
    ax1.set_ylim(-0.6, 0.9)
    ax1.set_zlim(-0.6, 0.9)

    '''Plot 2: '''
    '''Visualizing the points after deprojection with respect to the first camera reference frame: '''
    fig2 = plt.figure()
    ...
    ax2.scatter(x_points_deprojected_points, y_points_deprojected_points, z_points_deprojected_points, s = 0.2)
    
    '''Base reference Frame: '''
    cloud_object_deprojected_points.R = cloud_object_deprojected_points.R_base
    cloud_object_deprojected_points.p = cloud_object_deprojected_points.p_base
    print('cloud_object_deprojected_points.R_base: ', cloud_object_deprojected_points.R_base)
    print('cloud_object_deprojected_points.p_base: ', cloud_object_deprojected_points.p_base)
    cloud_object_deprojected_points.scale_value = 0.45
    cloud_object_deprojected_points.length_value = 0.25
    ax2 = cloud_object_deprojected_points.plotReferenceFrames(ax2)

    '''Camera reference Frame:'''
    cloud_object_deprojected_points.R = R_pose_1
    cloud_object_deprojected_points.p = p_pose_1
    cloud_object_deprojected_points.scale_value = 0.25
    cloud_object_deprojected_points.length_value = 0.15
    ax2 = cloud_object_deprojected_points.plotReferenceFrames(ax2)
    ...
    '''Plot 3: '''
    '''Visualizing both set of points and reference frames together: '''
    fig3 = plt.figure()
    ...
    ax3.scatter(x_points_deprojected_points, y_points_deprojected_points, z_points_deprojected_points, s = 0.2)
    ax3.scatter(x_points_transformed_points, y_points_transformed_points, z_points_transformed_points, s = 0.2, color='r')
    ...
    cloud_object_deprojected_points.scale_value = 0.45
    cloud_object_deprojected_points.length_value = 0.25
    ax3 = cloud_object_deprojected_points.plotReferenceFrames(ax3)

    '''Camera reference Frame Pose 1:'''
    cloud_object_deprojected_points.R = R_pose_1
    cloud_object_deprojected_points.p = p_pose_1
    cloud_object_deprojected_points.scale_value = 0.25
    cloud_object_deprojected_points.length_value = 0.15
    ...
    '''Camera reference Frame Pose 2:'''
    cloud_object_transformed_points.R = R_pose_2
    cloud_object_transformed_points.p = p_pose_2
    cloud_object_transformed_points.scale_value = 0.25
    cloud_object_transformed_points.length_value = 0.15
    ax3 = cloud_object_transformed_points.plotReferenceFrames(ax3)

    ax3.set_xlim(-0.6, 0.9)
    ax3.set_ylim(-0.6, 0.9)
    ax3.set_zlim(-0.6, 0.9)
